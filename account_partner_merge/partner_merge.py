...
class MergePartnerAutomatic(orm.TransientModel):
    ...
    def _update_values(self,  src_partners, dst_partner, context=None):
        ...
        super(MergePartnerAutomatic, self)._update_values(
             src_partners, dst_partner, context=context
        )

        invoice_obj = self.pool.get('account.invoice')
        invoice_ids = invoice_obj.search(
             [('partner_id', '=', dst_partner.id)], context=context
        )
        # call write to refresh stored value
        invoice_obj.write( invoice_ids, {}, context=context)
